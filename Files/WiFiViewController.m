...
- (void)viewDidLoad {
    ...
    warnLabel.frame = CGRectMake(20, 320, [UIScreen mainScreen].bounds.size.width - 40, 250);
    ...
    netLabel.text = [NSString stringWithFormat:@"http://%@:9527", ipAddr];
    NSLog(@"%@", netLabel.text);
...
- (void)initHttpServer
{
    ...
    NSString *webPath = [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingString:@"/web"];
...
- (void)startServer
{
    ...
    if([httpServer start:&error])
    {
        ...
        warnLabel.text = [NSString stringWithFormat:@"提示：\nWiFi传文件功能已经开启\n传输过程中请勿关闭此页或者锁屏\nStarted HTTP Server on port %hu", [httpServer listeningPort]];
    }
    else
    {
        DDLogError(@"Error starting HTTP Server: %@", error);
        warnLabel.text = [NSString stringWithFormat:@"提示：\nWiFi传文件功能已经开启\n传输过程中请勿关闭此页或者锁屏\nError starting HTTP Server: %@", error];
    }
}
